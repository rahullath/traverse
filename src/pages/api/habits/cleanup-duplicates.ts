  ...
  try {
    ...
    for (const [habitName, habitList] of habitGroups) {
      if (habitList.length > 1) {
        console.log(
          `🔄 Processing duplicates for "${habitName}": ${habitList.length} copies`,
        );
        ...
        console.log(
          `  ❌ Deleting: ${deleteHabits.map((h: any) => `${h.id} (${h.created_at})`).join(", ")}`,
        );
      } else {
    ...
    summary.deleted_habits = toDelete.map((h: any) => ({
      id: h.id,
      name: h.name,
      created_at: h.created_at,
    }));
    ...
    if (entriesError) {
      ...
      console.log(
        `✅ Deleted habit entries for ${habitIdsToDelete.length} duplicate habits`,
      );
    }
