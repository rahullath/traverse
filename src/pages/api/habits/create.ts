...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (
      (measurement_type === "count" || measurement_type === "duration") &&
      (!target_value || target_value <= 0)
    ) {
      return new Response(
        JSON.stringify({
          error: "Target value is required for count and duration habits",
        }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: "Target unit is required for count and duration habits",
        }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
    ...
    if (existingHabit) {
      return new Response(
        JSON.stringify({ error: "A habit with this name already exists" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
