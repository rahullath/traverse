...
function generateSummary(habits: any[], entries: any[]) {
  ...
  const bestStreakHabit = habitStreaks.find(
    (h) => h.maxStreak === longestStreak,
  );
  ...
  const habitCompletionRates = habits.map((habit) => {
    ...
    const rate =
      habitEntries.length > 0
        ? habitEntries.filter((e) => e.value === 1).length / habitEntries.length
        : 0;
    return {
  ...
  const mostConsistentHabit = habitCompletionRates.reduce(
    (prev, current) =>
