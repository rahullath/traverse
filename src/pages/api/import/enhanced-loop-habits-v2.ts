...
function asImportSummaryFromPerHabit(
  result: {
    success: boolean;
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    const importFormat =
      (formData.get("importFormat") as string | null) || "root";
    const conflictResolutionsStr = formData.get("conflictResolutions") as
      | string
      | null;
      ...
      start(controller) {
        ...
        if (importFormat === "per-habit") {
          ...
          for (const [key, value] of formData.entries()) {
            ...
            if (key === "importFormat" || key === "conflictResolutions")
              continue;
            ...
            if (
              !lowerName.endsWith("/checkmarks.csv") &&
              lowerName !== "checkmarks.csv"
            )
              continue;
          ...
          importer
            ...
            .then((result) => {
              ...
              const summary = asImportSummaryFromPerHabit(
                result,
                Date.now() - startedAt,
              );
              sendProgress({
            ...
            .catch((error: any) => {
              ...
              send({
                type: "error",
                message: error?.message || "Per-habit import failed",
              });
              controller.close();
            });
