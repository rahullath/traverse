...
export const POST: APIRoute = async ({ request }) => {
  const supabaseUrl = getEnvValue(
    normalize(import.meta.env.PUBLIC_SUPABASE_URL),
    normalize(process.env.PUBLIC_SUPABASE_URL),
  );
  const serviceRoleKey = getEnvValue(
    normalize(import.meta.env.SUPABASE_SERVICE_ROLE_KEY),
    normalize(process.env.SUPABASE_SERVICE_ROLE_KEY),
  );
  const webhookSecret = getEnvValue(
    normalize(import.meta.env.STRIPE_WEBHOOK_SECRET),
    normalize(process.env.STRIPE_WEBHOOK_SECRET),
  );
