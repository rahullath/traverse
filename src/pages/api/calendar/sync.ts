...
export const POST: APIRoute = async ({ url, cookies }) => {
  try {
    ...
    if (sourceId) {
      ...
      if (sourcesError) {
        throw new Error(
          `Failed to fetch calendar source: ${sourcesError.message}`,
        );
      }

      if (!sources || sources.length === 0) {
        return new Response(
          JSON.stringify({ error: "Calendar source not found" }),
          {
            status: 404,
            headers: { "Content-Type": "application/json" },
          },
      ...
      if (source.type === "ical" && source.url) {
        ...
        const newEvents = icalParser.convertToCalendarEvents(
          parsedData,
          source,
          user.id,
        );
        ...
        for (const newEvent of newEvents) {
          ...
          if (existingEvent) {
            ...
            const hasChanged =
              ...
              new Date(existingEvent.start_time).getTime() !==
                newEvent.start_time.getTime() ||
              new Date(existingEvent.end_time).getTime() !==
                newEvent.end_time.getTime() ||
            ...
            // Add new event
            await serverAuth.supabase.from("calendar_events").insert(newEvent);
            eventsAdded++;
          }
        }
      ...
      if (sourcesError) {
        throw new Error(
          `Failed to fetch calendar sources: ${sourcesError.message}`,
        );
      }
