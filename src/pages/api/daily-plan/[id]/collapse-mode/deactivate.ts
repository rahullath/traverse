...
import { getDailyPlan, getDailyPlanByDate } from '../../../../../lib/daily-plan/database';
...
export const POST: APIRoute = async ({ params, cookies }) => {
  try {
    ...
    return new Response(JSON.stringify({
      ok: true,
      collapse_mode: collapseMode,
      message: 'Collapse mode deactivated',
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error deactivating collapse mode:', error);
    return new Response(JSON.stringify({
      error: 'Failed to deactivate collapse mode',
