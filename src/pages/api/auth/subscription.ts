...
export const GET: APIRoute = async ({ cookies }) => {
  try {
    ...
    const trialEndDate = preferences.trial_end_date
      ? new Date(preferences.trial_end_date)
      : null;
    const now = new Date();
    ...
    const subscriptionStatus = {
      ...
      daysLeft: daysLeft,
      isTrialActive:
        daysLeft > 0 && preferences.subscription_status === "trial",
      ...
      case "extend_trial":
        ...
        if (currentPrefs) {
          const currentEndDate = new Date(
            currentPrefs.trial_end_date || Date.now(),
          );
