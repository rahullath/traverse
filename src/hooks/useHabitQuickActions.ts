...
export function useHabitQuickActions(habits: Habit[]) {
  ...
  const handleEnhancedLog = useCallback(
...
function showToast(message: string) {
  ...
  toast.className =
    "fixed top-4 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm max-w-sm text-center";
