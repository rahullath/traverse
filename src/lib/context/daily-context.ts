...
function aggregateSubstances(
  entries: HabitEntry[],
): DailyContext["substances"] {
  const nicotineEntries = entries.filter(
    (e) =>
      e.notes?.toLowerCase().includes("pouch") ||
      e.notes?.toLowerCase().includes("nicotine"),
  );

  const cannabisEntries = entries.filter(
    (e) =>
  ...
  const caffeineEntries = entries.filter(
    (e) =>
  ...
  for (const entry of cannabisEntries) {
    if (entry.notes) {
      ...
      // Extract method
      const methodMatch = entry.notes.match(
        /(vaporizer|bong|edible|avb|joint)/i,
      );
  ...
  for (const entry of caffeineEntries) {
    if (entry.notes) {
      const drinkMatch = entry.notes.match(
        /(monster|lucozade|red bull|energy drink)(?:\s+(ultra\s+\w+|original|sugar\s+free))?/i,
      );
      if (drinkMatch) {
  ...
  return {
    nicotine: {
      ...
      strength_mg:
        strengthCount > 0 ? totalStrength / strengthCount : undefined,
...
function aggregateMeds(entries: HabitEntry[]): DailyContext["meds"] {
  const medsEntries = entries.filter(
    (e) =>
...
function aggregateHygiene(entries: HabitEntry[]): DailyContext["hygiene"] {
  const showerEntries = entries.filter(
    (e) =>
  ...
  const oralEntries = entries.filter(
    (e) =>
  ...
  const skincareEntries = entries.filter(
    (e) =>
  ...
  for (const entry of showerEntries) {
    if (entry.notes) {
      const typeMatch = entry.notes.match(
        /(reg(?:ular)?\s*shower|head\s*shower|proper\s*cleanse|only\s*water)/i,
      );
      if (typeMatch) {
  ...
  return {
    ...
    reliability: calculateReliability(
      [...showerEntries, ...oralEntries, ...skincareEntries],
      "hygiene",
      7,
    ),
  };
 ...
 */
function calculateDurationPriors(
  entries: HabitEntry[],
): DailyContext["duration_priors"] {
  ...
  for (const entry of entries) {
    if (entry.duration_minutes && entry.duration_minutes > 0) {
      ...
      if (
        notes.includes("hygiene") ||
        notes.includes("oral") ||
        notes.includes("teeth")
      ) {
...
function calculateReliability(
  ...
  const consistencyScore =
    values.length > 1 ? 1.0 - variance(values) / Math.max(...values, 1) : 0.5;
...
export async function generateDailyContext(
  ...
  const primaryEntries =
    yesterdayEntries.length > 0 ? yesterdayEntries : trailingEntries;
  ...
  if (yesterdayEntries.length === 0) {
    context.substances.nicotine.reliability = Math.min(
      0.4,
      context.substances.nicotine.reliability,
    );
    context.substances.cannabis.reliability = Math.min(
      0.4,
      context.substances.cannabis.reliability,
    );
    context.substances.caffeine.reliability = Math.min(
      0.4,
