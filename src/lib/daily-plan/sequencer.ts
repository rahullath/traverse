...
export function getCurrentBlockFromPlan(plan: DailyPlan): TimeBlock | null {
  ...
  // Find first block with status='pending'
  const currentBlock = plan.timeBlocks.find(
...
export async function getNextBlocks(
  ...
  for (
    ...
    i < timeBlocks.length && nextBlocks.length < count;
    i++
  ) {
 ...
 * @returns Array of next pending blocks (up to count)
 */
export function getNextBlocksFromPlan(
  plan: DailyPlan,
  count: number,
): TimeBlock[] {
  if (!plan.timeBlocks || plan.timeBlocks.length === 0) {
  ...
  // Find current block
  const currentIndex = plan.timeBlocks.findIndex(
    (block) => block.status === "pending",
  );
  ...
  for (
    let i = currentIndex + 1;
    i < plan.timeBlocks.length && nextBlocks.length < count;
    i++
  ) {
