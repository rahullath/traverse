...
const PATTERNS = {
  ...
  // Shower patterns
  shower_type:
    /(reg(?:ular)?\s*shower|head\s*shower|proper\s*cleanse|only\s*water|complete\s*cleanse)/i,
...
export function parseNote(
  ...
  try {
    ...
    const effectiveType =
      semanticType || detectSemanticTypeFromNote(processedNote);

    if (
      effectiveType === SemanticType.NICOTINE_POUCHES ||
    ...
    if (
      effectiveType === SemanticType.POT_USE ||
    ...
    if (
      effectiveType === SemanticType.ENERGY_DRINK ||
    ...
    if (
      effectiveType === SemanticType.SHOWER ||
    ...
    if (
      effectiveType === SemanticType.ORAL_HYGIENE_SESSIONS ||
    ...
    if (
      effectiveType === SemanticType.SKINCARE ||
    ...
    if (
      effectiveType === SemanticType.SLEEP_PROXY ||
    ...
    if (
      processedNote.match(PATTERNS.social_context) ||
...
function detectSemanticTypeFromNote(note: string): SemanticType | null {
  ...
  if (lowerNote.match(/monster|lucozade|red bull|energy drink/))
  ...
  if (lowerNote.match(/oral|teeth|brush|floss/))
    return SemanticType.ORAL_HYGIENE_SESSIONS;
...
function parseNicotineData(note: string, result: ParsedNoteData): void {
  const method = note.match(/vap|puff/i)
    ? "vape"
    : note.match(/pouch/i)
      ? "pouch"
      : "other";
...
function parseCannabisData(note: string, result: ParsedNoteData): void {
  ...
  let method: "vaporizer" | "bong" | "edibles" | "avb" | "joint" | "other" =
    "other";
...
function parseCaffeineData(note: string, result: ParsedNoteData): void {
  ...
  if (productMatch) {
    result.caffeine = {
      ...
      brand: brandMatch
        ? brandMatch[1].toLowerCase().replace(/\s+/g, " ")
        : undefined,
    };
  }
}
...
function parseShowerData(note: string, result: ParsedNoteData): void {
  ...
  let type: "reg_shower" | "head_shower" | "proper_cleanse" | "only_water" =
    "reg_shower";
