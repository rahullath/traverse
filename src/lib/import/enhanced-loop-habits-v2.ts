...
export function detectImportFormat(files: File[]): ImportFormat {
  ...
  const checkmarksCount = fileNames.filter(
    (name) => name === "checkmarks.csv",
  ).length;
 ...
 * Requirements: 1.2
 */
export function extractNotesFromPerHabit(
...
export class EnhancedLoopHabitsImporterV2 {
  ...
  private async processPerHabitFile(
    ...
    const dedupedByDate = new Map<
      string,
      { date: string; valueRaw: string; notes: string | null }
    >();
    ...
    if (entryDates.length > 0) {
      const { data: existingRows, error: existingError } = await this.supabase
        .from("habit_entries")
        .select(
          "id, date, value, notes, parsed, numeric_value, source, logged_at",
        )
        .eq("user_id", this.userId)
        .eq("habit_id", habitId)
        .in("date", entryDates);
  ...
  private inferValueMode(
    ...
    if (
      normalizedMeasurement === "count" ||
      normalizedMeasurement === "duration" ||
      normalizedMeasurement === "numerical"
    ) {
      return "NUMERICAL";
    }
