...
export class EnhancedLoopHabitsImporter {
  ...
  private updateProgress(
    ...
    console.log(
      `📊 [${stage}] ${progress}% - ${message}${details ? ` (${details})` : ""}`,
    );
  }
  ...
  async importWithEnhancedHandling(
    ...
    try {
      ...
      summary.totalEntries = Object.values(parsedData.checkmarks).reduce(
        (total, habitEntries) => total + Object.keys(habitEntries).length,
        0,
      );
      ...
      this.updateProgress(
        "conflict_resolution",
        40,
        "Detecting conflicts with existing habits",
      );
      ...
      const resolvedHabits = this.applyConflictResolutions(
        parsedData.habits,
        conflicts,
        conflictResolutions || [],
      );
      ...
      const importResult = await this.importHabitsAndEntries(
        resolvedHabits,
        parsedData,
      );
  ...
  private async validateCSVFiles(csvFiles: {
    habits: string;
    checkmarks: string;
    scores: string;
    ...
    const checkmarksValidation = this.validateCheckmarksCSV(
      csvFiles.checkmarks,
    );
  ...
  private validateHabitsCSV(csv: string): {
    errors: ImportError[];
    warnings: ImportError[];
  } {
    const errors: ImportError[] = [];
  ...
  private validateCheckmarksCSV(csv: string): {
    errors: ImportError[];
    warnings: ImportError[];
  } {
    const errors: ImportError[] = [];
    ...
    if (lines.length < 2) {
      warnings.push({
        ...
        severity: "warning",
        message:
          "Checkmarks CSV has no data rows - no habit entries will be imported",
      });
      return { errors, warnings };
    }
  ...
  private validateScoresCSV(csv: string): {
    errors: ImportError[];
    warnings: ImportError[];
  } {
    const errors: ImportError[] = [];
  ...
  private crossValidateFiles(csvFiles: {
    habits: string;
    checkmarks: string;
    scores: string;
    ...
    try {
      ...
      const checkmarksHabits = checkmarksHeader
        .split(",")
        .slice(1)
        .map((h) => h.trim());

      // Find mismatches
      const missingInCheckmarks = Array.from(habitNames).filter(
        (name) => !checkmarksHabits.includes(name),
      );
      const extraInCheckmarks = checkmarksHabits.filter(
        (name) => name && !habitNames.has(name),
      );
  ...
  private async parseCSVFiles(csvFiles: {
    habits: string;
    checkmarks: string;
    scores: string;
  }) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        for (let j = 1; j < values.length && j < header.length; j++) {
          ...
          data[habitName][dateStr] = this.normalizeHabitValue(
            rawValue,
            habitName,
          );
        }
  ...
  private async detectConflicts(
    ...
    for (const importedHabit of importedHabits) {
      const existingHabit = existingHabits.find(
        (h) => h.name.toLowerCase() === importedHabit.name.toLowerCase(),
      );
  ...
  private applyConflictResolutions(
    ...
    for (const resolution of resolutions) {
      const habitIndex = resolvedHabits.findIndex(
        (h) => h.name === resolution.habitName,
      );
  ...
  private async importHabitsAndEntries(habits: any[], parsedData: any) {
    ...
    for (const habit of habits) {
      try {
        ...
        if (insertedHabit) {
          ...
          console.log(
            `✅ Created new habit: ${habit.name} (${insertedHabit.id})`,
          );
          importedHabits++;
        } else {
          skippedHabits++;
    ...
    for (const [habitName, dateEntries] of Object.entries(
      parsedData.checkmarks,
    )) {
      const habitId = habitMap.get(habitName);
      if (!habitId) continue;

      for (const [dateStr, value] of Object.entries(
        dateEntries as Record<string, number>,
      )) {
        if (value !== null && value !== undefined && value >= 0) {
          try {
            ...
            if (existingEntriesMap.has(entryKey)) {
              console.log(
                `📝 Skipping existing entry: ${habitName} on ${dateStr}`,
              );
              continue;
            }
    ...
    if (entries.length > 0) {
      console.log(
        `📝 About to insert ${entries.length} entries. First entry sample:`,
        entries[0],
      );
      ...
      try {
        ...
        console.log(
          `📦 Inserting ${entries.length} entries using safe insertion (prevents duplicates)`,
        );
        let insertedCount = 0;
          ...
          try {
            ...
            if (useSafeInsertRpc) {
              ...
              if (error?.code === "PGRST202") {
                ...
                console.warn(
                  "Safe insert RPC not found, falling back to direct inserts",
                );
                errors.push({
                  type: "database",
                  severity: "warning",
                  message:
                    "Safe insert RPC not found; using direct inserts for this import.",
                  details: error,
                });
                error = null;
              }
            ...
            if (error) {
              ...
              if ((i + 1) % 100 === 0) {
                console.log(
                  `✅ Processed ${i + 1}/${entries.length} entries (${insertedCount} inserted/updated)`,
                );
              }
            }
          } catch (insertError: any) {
            console.error(
              `❌ Exception inserting entry ${i + 1}:`,
              insertError,
            );
            errors.push({
              type: "database",
        ...
        if (insertedCount > 0) {
          console.log(
            `✅ Successfully processed ${importedEntries} habit entries (inserted/updated with duplicate prevention)`,
          );
        }
      } catch (error: any) {
  ...
  private async calculateAllStreaks() {
    ...
    for (const habit of habits) {
      ...
      const { currentStreak, bestStreak, totalCompletions } =
        this.calculateStreakForHabit(entries, habit.name, habit.type);
  ...
  private calculateStreakForHabit(
    entries: any[],
    habitName: string,
    habitType: string,
  ) {
  ...
  private async generateStatistics() {
    ...
    return {
      ...
      entriesByMonth,
      averageStreakLength:
        habits.length > 0 ? totalStreakLength / habits.length : 0,
      mostActiveHabit,
    };
  }
  ...
  private generateRecommendations(summary: ImportSummary): string[] {
    ...
    if (summary.importedHabits > 0) {
      recommendations.push(
        `✅ Successfully imported ${summary.importedHabits} habits with ${summary.importedEntries} entries`,
      );
    }

    if (summary.conflicts.length > 0) {
      recommendations.push(
        `🔄 Resolved ${summary.conflicts.length} naming conflicts - review merged habits`,
      );
    }

    if (summary.warnings.length > 0) {
      recommendations.push(
        `⚠️ ${summary.warnings.length} warnings encountered - check data quality`,
      );
    }

    if (summary.statistics.averageStreakLength > 7) {
      recommendations.push(
        `🔥 Great streak performance! Average streak: ${Math.round(summary.statistics.averageStreakLength)} days`,
      );
    }

    if (summary.statistics.mostActiveHabit) {
      recommendations.push(
        `🏆 Most consistent habit: ${summary.statistics.mostActiveHabit}`,
      );
    }

    recommendations.push(
      `📊 Visit the Analytics dashboard to explore your habit patterns`,
    );
    recommendations.push(
      `🎯 Consider setting up habit reminders for better consistency`,
    );
  ...
  private categorizeHabit(name: string | undefined | null): string {
    ...
    if (lower.includes("valorant") || lower.includes("game"))
      return "Entertainment";
    return "General";
  }

  private determineHabitType(
  ...
  private determineMeasurementType(
    habit: any,
  ): "boolean" | "count" | "duration" | "rating" {
