...
export class RoutineService {
   ...
   */
  async getMorningRoutineOptimized(
   ...
   */
  async getSubstanceReductionGoal(
    substanceType: "vaping" | "smoking",
  ): Promise<any | null> {
    const { data, error } = await this.supabase
  ...
  async recordMissedActivity(
    ...
    // Record the miss
    await this.supabase.from("uk_student_routine_misses").insert({
      user_id: this.userId,
      routine_id: routineId,
      reason,
    ...
    const strategies = await this.getRecoveryStrategies(
      routine?.routine_type || "general",
    );
  ...
  async getRoutineStats(routineId: string, days: number = 30): Promise<any> {
    ...
    return {
      ...
      consistency_score: Math.round(
        completionRate * 0.8 + (1 - missedDays / totalDays) * 0.2 * 100,
      ),
    };
  }
  ...
  async getDailyPlan(date: Date): Promise<any> {
    ...
    return {
      ...
      routines: routines.map((r) => ({
        ...r,
        completed:
          completions?.some((c: any) => c.routine_id === r.id) || false,
      })),
      academic_events: academicEvents || [],
      total_planned_time: routines.reduce(
        (sum, r) => sum + r.estimated_duration,
        0,
      ),
    };
  }
