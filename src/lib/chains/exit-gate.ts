...
export class ExitGateService {
  ...
  evaluateGate(): ExitGate {
    ...
    const status: ExitGateStatus =
      unsatisfiedConditions.length > 0 ? "blocked" : "ready";
  ...
  static fromGateTags(gateTags: string[]): ExitGateService {
    const filteredConditions = DEFAULT_GATE_CONDITIONS.filter((condition) =>
      gateTags.includes(condition.id),
    );
