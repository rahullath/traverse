...
export class AnchorService {
  ...
  async getAnchorsForDate(
    ...
    try {
      ...
      const calendarAnchors = events.map((event) =>
        this.convertEventToAnchor(event),
      );
      ...
      if (supabaseClient) {
        ...
        if (!manualError && Array.isArray(manualRows)) {
          ...
          console.warn(
            "[Anchor Service] Manual anchors query failed, continuing with calendar anchors only:",
            manualError.message,
          );
        }
      }
      ...
      console.error(
        "[Anchor Service] Calendar API error - returning empty anchors array:",
        error,
      );
  ...
  classifyAnchorType(event: CalendarEvent): AnchorType {
    ...
    // Check for seminar keywords
      ...
      this.containsAnyKeyword(combinedText, this.config.seminarKeywords)
    ) {
      type = "seminar";
    }
    // Check for appointment keywords
    else if (
      this.containsAnyKeyword(combinedText, this.config.appointmentKeywords)
    ) {
      type = "appointment";
    }
  ...
  isTimeInAnchor(anchors: Anchor[], time: Date): boolean {
    return anchors.some((anchor) => time >= anchor.start && time <= anchor.end);
  }
}
