...
export class CalendarService {
  ...
  async deleteCalendarSource(sourceId: string): Promise<void> {
    // Delete events first
    await supabase.from("calendar_events").delete().eq("source_id", sourceId);
    ...
    try {
      ...
      for (const newEvent of newEvents) {
        ...
        if (existingEvent) {
          ...
          await supabase.from("calendar_events").insert(insertPayload);
          result.events_added++;
        }
      }
  ...
  private async syncICalSource(
    ...
    return icalParser.convertToCalendarEvents(
      parsedData,
      source,
      source.user_id,
    );
  ...
  private async syncGoogleSource(
    ...
    if (
      ...
      const refreshed = await googleCalendar.refreshAccessToken(
    ...
    const response = await googleCalendar.fetchEvents(
      accessToken,
      source.credentials.calendar_id,
      {
        timeMin,
        timeMax,
        maxResults: 2500,
  ...
  async findAvailableSlots(
    ...
    while (current < query.end && slotCount < 500) {
      // Increased limit to find more slots
      ...
      current = new Date(
        current.getTime() + Math.max(30, buffer || 30) * 60000,
      );
    }

    console.log(
      `🔍 Slot search results: ${slotCount} slots checked, ${availableCount} available`,
    );
  ...
  async getCalendarStats(userId: string): Promise<CalendarStats> {
    ...
    const sources = await this.getCalendarSources(userId);
    const eventsBySource = allEvents.reduce(
      (acc, event) => {
  ...
  private calculateConflictSeverity(
    ...
    const hasHighImportance =
      event1.importance === "critical" || event2.importance === "critical";
    const hasFixedEvents =
      event1.flexibility === "fixed" || event2.flexibility === "fixed";
  ...
  async scheduleTask(
    ...
    if (preferred_start_time) {
      ...
      actualSearchStart =
        preferredTime > now
          ? preferredTime
          : new Date(now.getTime() + 30 * 60000); // 30 mins from now if preferred time is past
      ...
      if (preferred_end_time) {
        ...
        searchEnd = new Date(
          actualSearchStart.getTime() + 7 * 24 * 60 * 60 * 1000,
        );
      }
    } else {
      ...
      searchEnd = deadline
        ? new Date(deadline)
        : new Date(actualSearchStart.getTime() + 21 * 24 * 60 * 60 * 1000);
    }
    ...
    if (availableSlots.length === 0) {
      ...
      throw new Error(
        "No available slots found for the given task and preferences.",
      );
    }
    ...
    const newCalendarEvent: TablesInsert<"calendar_events"> = {
      user_id,
      source_id:
        source_id || (await this.getDefaultCalendarSourceId(user_id, client)), // Use a default source if not provided
      title,
      description,
    ...
    if (error) {
      throw new Error(
        `Failed to create calendar event for task: ${error.message}`,
      );
    }
    ...
    const scheduledTask: ScheduledTask = {
      ...
      resolution_details:
        conflicts.length > 0
          ? "Conflicts detected after scheduling"
          : undefined,
    };
   ...
   */
  private async getDefaultCalendarSourceId(
    userId: string,
    supabaseClient?: any,
  ): Promise<string> {
    const sources = await this.getCalendarSources(userId, supabaseClient);
    let defaultSource =
      sources.find((s) => s.type === "manual" && s.is_active) ||
      sources.find((s) => s.is_active);

    if (!defaultSource) {
      console.log(
        "🔧 No active calendar sources found, creating default manual calendar",
      );
      // Create a default manual calendar source
      defaultSource = await this.createCalendarSource(
        userId,
        {
          name: "My Tasks",
          type: "manual",
