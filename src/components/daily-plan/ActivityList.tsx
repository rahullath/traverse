...
export default function ActivityList({
  ...
  const currentBlock =
    visibleTimeBlocks.find((block) => block.status === "pending") || null;
  ...
  const isPlanComplete = () => {
    ...
    const hasPendingBlocks = blocksAfterNow.some(
    ...
    <div className="bg-surface rounded-xl border border-border shadow-lg">
      <div className="p-6 border-b border-border">
        ...
        <p className="text-sm text-text-muted mt-1">
          {visibleTimeBlocks.filter((b) => b.status === "completed").length} of{" "}
          {visibleTimeBlocks.length} activities completed
        </p>
      </div>
      ...
      <div className="p-6 space-y-4">
        ...
        {currentBlock ? (
              ...
              <div className="flex-1">
                <div className="flex items-center mb-2">
                  <span className="text-2xl mr-2">
                    {getActivityIcon(currentBlock.activityType)}
                  </span>
                  <div>
                ...
                <p className="text-sm text-text-muted ml-10 mt-1">
                  {currentBlock.activityType.charAt(0).toUpperCase() +
                    currentBlock.activityType.slice(1)}
          ...
          <div className="bg-accent-success/10 border border-accent-success rounded-lg p-4 text-center">
            ...
            <p className="text-sm text-text-muted mt-1">
              Great job finishing your plan for today
            </p>
          </div>
        ) : null}
