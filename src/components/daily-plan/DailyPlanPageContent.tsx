...
export default function DailyPlanPageContent() {
  ...
  type CollapseModeState = {
    active: boolean;
    trigger_reason?: CollapseTriggerReason;
    triggered_at?: string;
  ...
  const getCollapseModeState = (targetPlan: DailyPlan | null): CollapseModeState | null => {
    if (!targetPlan) return null;
    const raw = (targetPlan.metadata || {}) as Record<string, any>;
    const collapseMode = raw.collapse_mode as CollapseModeState | undefined;
    return collapseMode || null;
  };

  const collapseMode = getCollapseModeState(plan);

  const getCollapseRecoveryBlocks = (targetPlan: DailyPlan | null) => {
    if (!targetPlan?.timeBlocks) return [];
    return [...targetPlan.timeBlocks]
      .filter((block) => {
        const role = ((block.metadata || {}) as any).role;
        return role?.type === 'recovery' && role?.recovery_kind === 'collapse';
      })
      .sort((a, b) => {
  ...
  const handleRecoveryStepToggle = async (blockId: string, currentlyCompleted: boolean) => {
    try {
      setIsUpdating(true);
      setError(null);
      const endpointAction = currentlyCompleted ? 'uncomplete' : 'complete';
      const response = await fetch(`/api/time-blocks/${blockId}/${endpointAction}`, {
        method: 'POST',
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload.error || 'Failed to update recovery step');
      }
      await fetchTodaysPlan();
    } catch (err) {
      console.error('Error updating recovery step:', err);
      setError(err instanceof Error ? err.message : 'Failed to update recovery step');
    } finally {
      setIsUpdating(false);
    }
  };

  const handleActivateCollapseMode = async (triggerReason: CollapseTriggerReason = 'manual_chaos') => {
    if (!plan) return;
    try {
      setIsUpdating(true);
      setError(null);
      const response = await fetch(`/api/daily-plan/${plan.id}/collapse-mode/activate`, {
        method: 'POST',
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload.error || 'Failed to activate collapse mode');
      }
      await fetchTodaysPlan();
      setActiveTab('timeline');
    } catch (err) {
      console.error('Error activating collapse mode:', err);
      setError(err instanceof Error ? err.message : 'Failed to activate collapse mode');
    } finally {
      setIsUpdating(false);
    }
  };

  const handleDeactivateCollapseMode = async () => {
    if (!plan) return;
    try {
      setIsUpdating(true);
      setError(null);
      const response = await fetch(`/api/daily-plan/${plan.id}/collapse-mode/deactivate`, {
        method: 'POST',
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload.error || 'Failed to deactivate collapse mode');
      }
      await fetchTodaysPlan();
    } catch (err) {
      console.error('Error deactivating collapse mode:', err);
      setError(err instanceof Error ? err.message : 'Failed to deactivate collapse mode');
    } finally {
      setIsUpdating(false);
    }
  };

  const handleCompleteRecoveryChain = async () => {
    if (!plan) return;
    try {
      setIsUpdating(true);
      setError(null);
      const response = await fetch(`/api/daily-plan/${plan.id}/collapse-mode/recovery-complete`, {
        method: 'POST',
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload.error || 'Failed to complete recovery chain');
      }
      await fetchTodaysPlan();
    } catch (err) {
      console.error('Error completing recovery chain:', err);
      setError(err instanceof Error ? err.message : 'Failed to complete recovery chain');
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const collapseRecoveryBlocks = getCollapseRecoveryBlocks(plan);
  const completedRecoveryCount = collapseRecoveryBlocks.filter((block) => block.status === 'completed').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap justify-end gap-2">
        {collapseMode?.active ? (
          <>
            <button
              onClick={handleCompleteRecoveryChain}
              disabled={isUpdating || collapseRecoveryBlocks.length === 0}
            ...
            <button
              onClick={handleDeactivateCollapseMode}
              disabled={isUpdating}
              className="px-3 py-2 rounded-lg border border-border-primary text-text-primary text-sm font-medium hover:bg-surface-hover disabled:opacity-60"
            >
              Resume Normal Plan
            </button>
          </>
          ...
          <button
            onClick={() => handleActivateCollapseMode('manual_chaos')}
            disabled={isUpdating}
            className="px-3 py-2 rounded-lg border border-amber-400/50 text-amber-300 text-sm font-medium hover:bg-amber-400/10 disabled:opacity-60"
          >
            Today is chaos
          </button>
        )}
      ...
      {collapseMode?.active && (
        <div className="rounded-xl border border-amber-400/40 bg-amber-500/10 p-4 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="text-base font-semibold text-amber-200">Collapse Mode Active</h3>
              <p className="text-xs text-amber-100/80">
                Recovery chain: {completedRecoveryCount}/{collapseRecoveryBlocks.length} complete
              </p>
            </div>
            {collapseMode.trigger_reason && (
              <span className="px-2 py-1 text-xs rounded bg-amber-400/20 text-amber-200">
                Trigger: {collapseMode.trigger_reason.replace('_', ' ')}
              </span>
            )}
          </div>

          <div className="space-y-2">
            {collapseRecoveryBlocks.length === 0 ? (
              <p className="text-sm text-amber-100/80">Recovery steps are being prepared. Refresh if this persists.</p>
            ) : (
              collapseRecoveryBlocks.map((block) => (
                <label
                  key={block.id}
                  className="flex items-center gap-3 rounded-lg border border-amber-300/20 bg-black/10 px-3 py-2"
                >
                  <input
                    type="checkbox"
                    checked={block.status === 'completed'}
                    onChange={() => handleRecoveryStepToggle(block.id, block.status === 'completed')}
                    className="h-4 w-4 rounded border-amber-200/50 bg-transparent"
                  />
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm ${block.status === 'completed' ? 'text-amber-100/70 line-through' : 'text-amber-50'}`}>
                      {block.activityName}
                    </p>
                    <p className="text-[11px] text-amber-100/70">
                      {Math.max(1, Math.round((new Date(block.endTime).getTime() - new Date(block.startTime).getTime()) / 60000))} min
                    </p>
                  </div>
                </label>
              ))
