...
export default function TokenBalance() {
  ...
  const isNewUser =
    tokenData.isNewUser ||
    (tokenData.balance === 5000 && tokenData.totalSpent === 0);
  const isExpired = tokenData.isExpired || false;
  const daysRemaining =
    tokenData.daysRemaining !== undefined ? tokenData.daysRemaining : 30;
      ...
      <div className="flex items-center space-x-3 px-4 py-2 bg-gradient-to-r from-green-500/10 via-cyan-500/10 to-blue-500/10 rounded-lg border border-cyan-500/30">
        ...
        <div className="flex flex-col">
          <div className="flex items-center space-x-2">
            <span
              className={`text-lg font-bold bg-gradient-to-r ${isExpired ? "from-red-400 to-orange-400" : "from-green-400 to-cyan-400"} bg-clip-text text-transparent`}
            >
          ...
          <span className="text-xs text-gray-400">
            ...
            {isExpired
              ? "Trial expired"
              : tokenData.totalSpent > 0
                ? `Spent ${tokenData.totalSpent}`
