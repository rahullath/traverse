      ...
      habitEntries.forEach((entry, index) => {
        ...
        if (isSuccess) {
          if (!currentStreak) {
            ...
            if (
              previousDate &&
              entryDate.getTime() - previousDate.getTime() ===
                24 * 60 * 60 * 1000
            ) {
  ...
  const timelineData = useMemo(() => {
    ...
    return streakData.map((streak) => {
      ...
      const startOffset = Math.max(
        0,
        Math.ceil(
          (streakStart.getTime() - startDate.getTime()) / (24 * 60 * 60 * 1000),
        ),
      );
      const width =
        Math.ceil(
          (streakEnd.getTime() - streakStart.getTime()) / (24 * 60 * 60 * 1000),
        ) + 1;
    ...
    <div className="card">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          <div className="text-sm text-text-muted">
            {activeStreaks.length} active streak
            {activeStreaks.length !== 1 ? "s" : ""}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {timelineData.slice(0, 10).map((streak, index) => (
            <div
              key={`${streak.habitId}-${streak.startDate}`}
              className="space-y-2"
            >
      ...
      <div className="mt-6 pt-4 border-t border-border">
        <div className="flex items-center justify-between text-xs text-text-muted">
          ...
          <span>
            Timeline: {data.dateRange.start} to {data.dateRange.end}
          </span>
        </div>
      </div>
    </div>
