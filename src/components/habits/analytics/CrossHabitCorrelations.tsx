...
const CrossHabitCorrelations: React.FC<CrossHabitCorrelationsProps> = ({
  data,
}) => {
  const correlations = useMemo(() => {
    ...
    for (let i = 0; i < data.habits.length; i++) {
      for (let j = i + 1; j < data.habits.length; j++) {
        ...
        const habit1Entries = data.entries.filter(
          (e) => e.habit_id === habit1.id,
        );
        const habit2Entries = data.entries.filter(
          (e) => e.habit_id === habit2.id,
        );

        // Create date-based lookup for correlation calculation
        const habit1ByDate = new Map(
          habit1Entries.map((e) => [e.date, e.value > 0 ? 1 : 0]),
        );
        const habit2ByDate = new Map(
          habit2Entries.map((e) => [e.date, e.value > 0 ? 1 : 0]),
        );

        // Find common dates
        const commonDates = Array.from(habit1ByDate.keys()).filter((date) =>
          habit2ByDate.has(date),
        );
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500">
            Not enough data to calculate correlations. Track at least 2 habits
            for 5+ days to see patterns.
          </p>
        </div>
      </div>
    );
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="text-sm text-gray-600 space-y-1">
          <p>
            <strong>Positive correlation:</strong> Habits that tend to happen
            together
          </p>
          <p>
            <strong>Negative correlation:</strong> Habits that tend to compete
            with each other
          </p>
          <p>
            <strong>Strength:</strong> Weak (&lt;40%), Moderate (40-70%), Strong
            (&gt;70%)
          </p>
        </div>
      </div>
