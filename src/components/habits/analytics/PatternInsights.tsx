...
export function PatternInsights({
  ...
  const analyzeStreakPatterns = (data: AnalyticsData): Insight | null => {
    ...
    const avgStreak =
      habitStreaks.reduce((sum, h) => sum + h.maxStreak, 0) /
      habitStreaks.length;
  ...
  const analyzeContextPatterns = (data: AnalyticsData): Insight | null => {
    ...
    if (moodStats.length >= 3) {
      const bestMood = moodStats.reduce((prev, current) =>
        current.success / current.total > prev.success / prev.total
          ? current
          : prev,
      );
  ...
  const analyzeDifficultyPatterns = (data: AnalyticsData): Insight | null => {
    const entriesWithEffort = data.entries.filter(
      (e) => e.effort !== undefined,
    );
    if (entriesWithEffort.length < 10) return null;

    const effortStats = [1, 2, 3, 4, 5]
      .map((effort) => {
        const effortEntries = entriesWithEffort.filter(
          (e) => e.effort === effort,
        );
        return {
          effort,
    ...
    if (effortStats.length >= 3) {
      ...
      if (highEffortEntries.length > 0 && lowEffortEntries.length > 0) {
        const highEffortRate =
          highEffortEntries.reduce((sum, e) => sum + e.success, 0) /
          highEffortEntries.reduce((sum, e) => sum + e.total, 0);
        const lowEffortRate =
          lowEffortEntries.reduce((sum, e) => sum + e.success, 0) /
  ...
  const generateRecommendations = (data: AnalyticsData): Insight[] => {
    ...
    const habitRates = data.habits
      .map((habit) => {
        const habitEntries = data.entries.filter(
          (e) => e.habit_id === habit.id,
        );
        const rate =
          habitEntries.length > 0
            ? habitEntries.filter((e) => e.value === 1).length /
              habitEntries.length
            : 0;
    ...
    if (entriesWithContext.length < data.entries.length * 0.3) {
      recommendations.push({
        ...
        description:
          "Only tracking context for some entries. Enhanced logging helps identify patterns and optimize your habits.",
        confidence: 0.6,
        actionable: true,
        icon: "📝",
        color: "text-accent-primary",
      ...
      <div className="card">
        <div className="text-center py-8">
          ...
          <p className="text-text-secondary">
            Keep tracking habits to unlock personalized insights and
            recommendations.
          </p>
        </div>
      </div>
            ...
            <div
              ...
              className={`p-4 rounded-lg border-l-4 ${
                insight.type === "achievement"
                  ? "bg-accent-success/5 border-accent-success"
        ...
        <div className="card">
          ...
          <div className="space-y-3 text-sm text-text-secondary">
            <div className="flex items-start space-x-2">
              ...
              <span>
                <strong>Patterns:</strong> Identified from your habit completion
                data and context
              </span>
            </div>
            <div className="flex items-start space-x-2">
              ...
              <span>
                <strong>Recommendations:</strong> Actionable suggestions to
            ...
            <div className="flex items-start space-x-2">
              ...
              <span>
                <strong>Achievements:</strong> Recognition of your progress and
                milestones
              </span>
            </div>
            <div className="flex items-start space-x-2">
              ...
              <span>
                <strong>Warnings:</strong> Areas that need attention or
                adjustment
              </span>
            </div>
          </div>
