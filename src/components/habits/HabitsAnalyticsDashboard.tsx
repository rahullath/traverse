...
export function HabitsAnalyticsDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "patterns" | "correlations" | "insights"
  >("overview");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (habitsScript && entriesScript) {
        ...
        const entries: HabitEntry[] = JSON.parse(
          entriesScript.textContent || "[]",
        );
        ...
        const successfulEntries = filteredEntries.filter(
          (entry) => entry.value === 1,
        ).length;
        const avgCompletionRate =
          totalEntries > 0 ? successfulEntries / totalEntries : 0;
        ...
        const habitConsistency = habits.map((habit) => {
          const habitEntries = filteredEntries.filter(
        ...
        const mostConsistent = habitConsistency.reduce(
          (prev, current) => (current.rate > prev.rate ? current : prev),
          { habit: "None", rate: 0 },
        );
