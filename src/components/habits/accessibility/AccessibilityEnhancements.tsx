  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (
        ...
        const habitCards = Array.from(
          document.querySelectorAll("[data-habit-id]"),
        );
      ...
      if (event.key === "Enter" || event.key === " ") {
        ...
        if (focusedElement?.hasAttribute("data-habit-id")) {
          ...
          const habitName =
            focusedElement.querySelector("h3, h4, .habit-name")?.textContent ||
            "Habit";
        ...
        modals.forEach((modal) => {
          if (!modal.classList.contains("hidden")) {
            const closeButton = modal.querySelector(
              '[data-close], .close-btn, button[aria-label*="close"]',
            );
  ...
  return (
    ...
    >
      ...
      <div id={`habit-${habit.id}-details`} className="sr-only">
        ...
        {habit.allows_skips
          ? " This habit allows skips."
          : " This is a strict habit."}
