...
export default function MobileQuickActions({
  ...
  const [swipeStates, setSwipeStates] = useState<
    Map<string, { startX: number; currentX: number; swiping: boolean }>
  >(new Map());
  const touchStartRef = useRef<
    Map<string, { x: number; y: number; time: number }>
  >(new Map());
  ...
  const syncOfflineQueue = async () => {
    ...
    if (successfulSyncs.length > 0) {
      setOfflineQueue((prev) =>
        prev.filter((_, index) => !successfulSyncs.includes(index)),
      );
  ...
  const showToast = (message: string) => {
    ...
    toast.className =
      "fixed top-4 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm";
  ...
  if (pendingHabits.length === 0) {
    return (
      <div
        className={`bg-slate-800/50 rounded-xl p-6 text-center ${className}`}
      >
    ...
    <div className={`bg-slate-800/50 rounded-xl p-4 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            {pendingHabits.length} pending • {completedToday}/{habits.length}{" "}
            done
          </p>
        </div>
      ...
      <div className="space-y-2">
        {pendingHabits.map((habit) => {
          ...
          const translateX = swipeState?.swiping
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  transform: `translateX(${Math.max(-100, Math.min(100, translateX))}px)`,
                }}
      ...
      {!batchMode && pendingHabits.length > 0 && (
        <div className="mt-4 text-center text-xs text-gray-500">
          💡 Swipe right to complete, left to{" "}
          {pendingHabits.some((h) => h.allows_skips) ? "skip" : "log details"}
        </div>
      )}
    </div>
  );
