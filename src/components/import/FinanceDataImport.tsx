    ...
    <div className="card p-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            accept=".csv"
            onChange={(e) =>
              e.target.files?.[0] && handleFileChange("bank", e.target.files[0])
            }
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              e.target.files?.[0] &&
              handleFileChange("crypto", e.target.files[0])
            }
            className="w-full px-3 py-2 bg-surface border border-border rounded-lg text-text-primary"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              e.target.files?.[0] &&
              handleFileChange("expenses", e.target.files[0])
            }
            className="w-full px-3 py-2 bg-surface border border-border rounded-lg text-text-primary"
          />
        </div>
            ...
            <div className="mt-1 space-y-1">
              {files.bank && (
                <div className="text-accent-success">
                  ✓ Bank Statement: {files.bank.name}
                </div>
              )}
              {files.crypto && (
                <div className="text-accent-success">
                  ✓ Crypto Holdings: {files.crypto.name}
                </div>
              )}
              {files.expenses && (
                <div className="text-accent-success">
                  ✓ Manual Expenses: {files.expenses.name}
                </div>
              )}
        ...
        <button
          ...
          disabled={
            isImporting || (!files.bank && !files.crypto && !files.expenses)
          }
      ...
      <div className="mt-6 p-4 bg-accent-warning/5 border border-accent-warning/20 rounded-lg">
        ...
        <div className="text-sm text-text-secondary space-y-1">
          <div>
            <strong>Crypto:</strong> 11 holdings worth $130.72 (USDC, TRX, SOL,
            ETH, etc.)
          </div>
          <div>
            <strong>Expenses:</strong> ~50+ transactions from Zepto, Swiggy,
            Blinkit, Supertails
          </div>
          <div>
            <strong>Categories:</strong> Food, Pet Care, Groceries, Fitness,
            Personal Care
          </div>
          <div>
            <strong>Total Value:</strong> ~₹25,000+ in expenses tracked
          </div>
        </div>
      </div>
    </div>
